...
[`TempCorrect`](@ref), [`WireCorrect`](@ref), [`InsulatedCorrect`](@ref)
...
struct InsulatedCorrect{U<:Real,Fluid} <: AbstractAnemCorrect
    "Fluid temperature"
    T::U
    "Operating resistance"
    Rw::U
...
"""
`InsulatedCorrect(T, Rw, Tw, P, fluid, ν, ϕ, n, β, c1, c2)`
`InsulatedCorrect(c::InsulatedCorrect, T, P, fluid, Rw, Tw)`
...
using `InsulatedCorrect`, see [`mf58correct`](@ref) for 
...
When only the fluid properties change, a new `InsulatedCorrect` object can be created
...
"""
function InsulatedCorrect(c::InsulatedCorrect, T, P, fluid, Rw, Tw)
    ...
    return InsulatedCorrect(T, Rw, Tw, P, fluid, ν, k*Pr^c.n, c.n, c.β, c.c1, c.c2)
    
end

correctmodel(mc::InsulatedCorrect, T, P, fluid, Rw, Tw) =
    InsulatedCorrect(mc, T, P, fluid, Rw, Tw) 
...
correct(E, mc_cal::InsulatedCorrect, T, P, fluid, Rw, Tw) = 
